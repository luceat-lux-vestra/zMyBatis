...
zMyBatis is a public Apache-2.0 repository. This document covers the mechanics of proposing a change. For what makes a change correct - MyBatis/SQL fidelity, DataGrip integration boundaries, CI trust-boundary rules, and review discipline - see [AGENTS.md](AGENTS.md), the actual engineering contract. This file exists to route contributors there and to state the parts of the process AGENTS.md doesn't cover.

## Before you start

- Read the [Apache-2.0 license](LICENSE). Contributions intentionally submitted for inclusion in zMyBatis are accepted under that license unless explicitly stated otherwise.
...
- Fork or branch from a fresh `main`. Note the base commit SHA if the change is part of a tracked hardening/reconciliation effort - PR descriptions in this repository record exact base and head SHAs for that reason.
...
- Treat fork pull requests as untrusted input. A contribution must never require repository secrets or write-scoped `GITHUB_TOKEN` authority merely to validate the submitted code.

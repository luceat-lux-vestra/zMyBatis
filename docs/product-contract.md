...
| Java `@Select/@Insert/@Update/@Delete` | `PsiJavaFile` + `PsiMethod`; extractor unit contract covers literal/array/reference shapes, and `AnnotationSqlExtractorProjectFixtureTest` exercises real Java PSI plus project-index-backed cross-file constant resolution | DEGRADED: parser/index-backed constant extraction is evidenced, but action-level caret/current-document selection and canonical mapper-method identity remain incomplete | SUPPORTED only with maintained Java parser/index evidence and canonical method identity | #62, #67 |
...
| Remembered parameter input | current UI can pre-fill last values using its current statement key | DEGRADED pending canonical identity/sensitive-data policy | persistence requires canonical statement identity, explicit retention/clearing rules, and no cross-statement/project bleed | #63, #67 |
...
- Java annotation literal/array/constant shapes with maintained project-backed parser/index evidence for cross-file constant resolution; action-level source authority and canonical mapper-method identity remain separate obligations;

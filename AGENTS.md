...
- A job in a `pull_request` workflow must never both check out source and hold `contents:write`, `checks:write`, or `pull-requests:write`. The default synthetic merge ref still contains PR-controlled changes, so R1 is about privileged PR code execution rather than the spelling of a `github.event.pull_request.head.*` ref. A job explicitly excluded from PR execution must make that exclusion auditable.

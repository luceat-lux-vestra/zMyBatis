...
- `build.yml` and `run-ui-tests.yml` default to `contents: read`; a write grant is job-scoped, minimal, and justified (see `merge-gate-policy.yml`'s `privilegedJobs`).
- A job that checks out `github.event.pull_request.head.*` (PR-authored source, not the synthetic merge ref) must never simultaneously hold `contents:write`, `checks:write`, or `pull-requests:write` - this is the exact shape the `inspectCode` (Qodana) job had before this contract existed, and is enforced as rule R1 in `check_trust_boundary.py`.
- A job holding none of those three write scopes must set `persist-credentials: false` on every `actions/checkout` step (rule R2, same tool). Relying on the checkout default leaves a usable token in `.git/config` for no reason.
...
- Required status-check contexts must be the jobs listed in `merge-gate-policy.yml`'s `requiredStatusChecks`, must run unconditionally on every ordinary PR (no `if:` guard, no `paths`/`paths-ignore` filter on the trigger), and must not silently rename out from under that list - `check_required_contexts.py` enforces the job/name half of that automatically.
